...
public class MavenService implements BuildService {
    ...
    public String executeBuild(String projectPath, boolean isTest) throws Exception {
        ...
        try {
            ...
            if (isTest) {
                builder.command("./mvnw", "clean", "install");
            } else {
                builder.command("./mvnw", "clean", "install", "-DskipTests");
            }
            ...
            if (exitCode != 0 || stdError.length() > 0) {
                throw new Exception("Maven build failed with exit code=" + exitCode + ". Error: " + stdError.toString());
            }
            ...
            logger.error("[ERROR] Failed maven build. exception={}, message={}", e.getClass().getName(), e.getMessage());
            e.printStackTrace();
            throw e;
        }
